// public/tally.mod.js (modular Firebase)
import {
  auth, db, functions,
  onAuthStateChanged,
  // firestore
...
async function verifyContractorUser() {
  ...
  if (!staffSnap.empty) {
    ...
    await updateDoc(
      doc(db, 'contractors', contractorId, 'staff', staffUid),
...
async function saveSessionToFirestore(showStatus = false) {
  ...
  try {
    const sessRef = doc(db, 'contractors', contractorId, 'sessions', firestoreSessionId);
...
async function loadSessionFromFirestore(id) {
    ...
    try {
        const sessRef = doc(db, 'contractors', contractorId, 'sessions', id);
...
async function markTourSeen(uid) {
  try {
    ...
    if (!uid) return;
    await setDoc(doc(db, 'users', uid), { hasSeenTour: true }, { merge: true });
  } catch (e) {
...
async function checkTourWelcomeStatus(uid) {
  ...
  // Try Firestore if we have a user
  if (uid) {
    try {
...
function startTally() {
  ...
  registerHelpDropdown();
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startTally);
} else {
  startTally();
}
